...
parser.add_argument("--ipd", type=float, default=0.0, 
                    help="Distanza interpupillare per VR (in unità della mesh). Se 0, video normale. Esempio tipico: 0.063 se la mesh è in metri.")
...
def zoom_camera(camera_view, zoom_factor):
    """
    Avvicina la telecamera al punto focale (zoom in).
    zoom_factor > 1.0 avvicina (es. 2.0 dimezza la distanza).
    """
    pos, focal, up = np.array(camera_view[0]), np.array(camera_view[1]), np.array(camera_view[2])
    view_vec = pos - focal
    # Divido il vettore distanza per il fattore di zoom
    new_pos = focal + (view_vec / zoom_factor)
    return (new_pos.tolist(), focal.tolist(), up.tolist())

def get_stereo_cameras(base_camera, ipd=0.0):
    """
    Calcola le posizioni Left e Right della telecamera basandosi sulla distanza interpupillare (IPD).
    IPD: La distanza tra gli occhi (in unità della simulazione, es. metri).
    """
    pos, focal, up = np.array(base_camera[0]), np.array(base_camera[1]), np.array(base_camera[2])
    
    # Calcola il vettore di vista e il vettore destra
    view_vec = focal - pos
    view_vec_norm = view_vec / np.linalg.norm(view_vec)
    up_norm = up / np.linalg.norm(up)
    right_vec = np.cross(view_vec_norm, up_norm)
    right_vec = right_vec / np.linalg.norm(right_vec)
    
    # Sposta le telecamere perpendicolarmente alla vista
    # Nota: Stiamo usando il metodo "Toe-in" (convergenza sul punto focale)
    # Per VR perfetto servirebbe "Off-axis projection", ma Toe-in è ok per video semplici.
    pos_left = pos - (right_vec * (ipd / 2.0))
    pos_right = pos + (right_vec * (ipd / 2.0))
    
    # Restituisce le due tuple camera complete (pos, focal, up)
    return (pos_left.tolist(), focal.tolist(), up.tolist()), \
           (pos_right.tolist(), focal.tolist(), up.tolist())
...
def render_single_frame_worker(frame_index, num_total_frames, iso4_filepath,
                               ...
                               particle_clim,
                               show_colorbar_flag,
                               vr_ipd=0.0): # <--- NUOVO PARAMETRO: vr_ipd
    """
    Renders a single frame. If vr_ipd > 0, renders Side-by-Side 3D.
    """
    
    # Determina se dobbiamo fare rendering stereo o mono
    is_stereo = vr_ipd > 0
    cameras = []
    
    if is_stereo:
        cam_l, cam_r = get_stereo_cameras(camera_view_for_frame, vr_ipd)
        cameras = [cam_l, cam_r]
    else:
        cameras = [camera_view_for_frame]

    temp_images = []

    # Loop 1 volta per Mono, 2 volte (Left/Right) per Stereo
    for cam_idx, cam_pos in enumerate(cameras):
        
        # Setup Plotter (uguale a prima)
        frame_plotter = pv.Plotter(off_screen=True, window_size=VIDEO_RESOLUTION)
        frame_plotter.background_color = BACKGROUND_COLOR
        
        # --- (Qui va tutto il codice di caricamento mesh/texture/particelle originale) ---
        # --- INCOLLA QUI IL CODICE DI CARICAMENTO MESH/TEXTURE DAL TUO SCRIPT ORIGINALE ---
        # Per brevità copio solo la logica base, assicurati di includere tutto il blocco try/except originale
        
        if passed_texture_exists_flag:
            try:
                worker_texture_object = pv.read_texture(passed_texture_image_path)
                frame_plotter.add_mesh(passed_terrain_mesh, texture=worker_texture_object, smooth_shading=True)
            except:
                frame_plotter.add_mesh(passed_terrain_mesh, color=passed_terrain_color, smooth_shading=True)
        else:
            frame_plotter.add_mesh(passed_terrain_mesh, color=passed_terrain_color, smooth_shading=True)
        ...
        try:
            ...
            if mesh4.n_points > 0: frame_plotter.add_mesh(mesh4, color="tan", opacity=0.7, smooth_shading=True)
            if mesh6.n_points > 0: frame_plotter.add_mesh(mesh6, color="lightblue", opacity=0.7, smooth_shading=True)
        ...
        if particle_csv_filepath and os.path.exists(particle_csv_filepath):
            try:
                ...
                if not df.empty and 'd' in df.columns:
                    ...
                    # Show the particle diameter colorbar according to the command-line option.
                    # In stereo/VR side-by-side mode, the colorbar must be rendered in both
                    # left and right views to keep the two eye images visually consistent.
                    show_scalar_bar = show_colorbar_flag
                    
                    actor = frame_plotter.add_mesh(glyphs, scalars='diameter_color', cmap=particle_cmap, clim=particle_clim, show_scalar_bar=False)
                    if show_scalar_bar and particle_clim is not None:
                         frame_plotter.add_scalar_bar(mapper=actor.mapper, title="Diameter", color='black')
            except Exception: pass
        
        # --- FINE BLOCCO CARICAMENTO ---

        frame_plotter.add_text(f"Time: {current_sim_time:.2f}s", font_size=15, position="upper_right", color="black")
        frame_plotter.camera_position = cam_pos
        
        # Salva in array numpy invece che su file
        img_array = frame_plotter.screenshot(transparent_background=False, return_img=True)
        temp_images.append(img_array)
        frame_plotter.close()

    # Salvataggio finale
    if is_stereo:
        # Unisci le immagini Side-by-Side (Sinistra a sinistra, Destra a destra)
        im_l = Image.fromarray(temp_images[0])
        im_r = Image.fromarray(temp_images[1])
        
        # Crea una nuova immagine larga il doppio
        dst = Image.new('RGB', (im_l.width + im_r.width, im_l.height))
        dst.paste(im_l, (0, 0))
        dst.paste(im_r, (im_l.width, 0))
        dst.save(output_png_filepath)
    else:
        # Salva normale
        Image.fromarray(temp_images[0]).save(output_png_filepath)
...
def create_animation_sequence(base_camera_view, rotation_angle_degrees=None, 
                              ...
                              interpolation_factor=1): # <--- NUOVO PARAMETRO (Default 1)
    """
    Gestisce il rendering. Se interpolation_factor > 1, crea frame intermedi per fluidificare la camera.
    """
    ...
    num_data_files = len(iso4_vtk_files)
    if num_data_files == 0:
        print("No frames to render. Skipping."); return

    # Calcoliamo il numero totale di frame video da generare
    total_video_frames = num_data_files * interpolation_factor
    
    print(f"\nPreparing sequence: '{sequence_base_name}'")
    if interpolation_factor > 1:
        print(f"  - INTERPOLATION ACTIVE: {interpolation_factor}x (Smooth Camera)")
    print(f"  - Total frames to render: {total_video_frames}")
    
    worker_tasks_list = []
    
    # Loop sui frame VIDEO
    for k in range(total_video_frames):
        
        # 1. Quale file dati usare? (Divisione intera)
        # Se factor=1, k=0 -> data=0. Se factor=3, k=0,1,2 -> data=0
        data_index = k // interpolation_factor
        
        current_sim_time = simulation_timesteps[data_index]
        iso4_path = iso4_vtk_files[data_index]
        iso6_path = iso6_vtk_files[data_index]
        particle_file = particle_data_map.get(current_sim_time, None)
        
        # 2. Quale angolo telecamera usare?
        current_camera_for_frame = base_camera_view
        
        if use_circular_orbit:
            # Qui usiamo 'k' e 'total_video_frames' per avere un movimento fluido frame-by-frame
            orbit_angle_deg = 360.0 * k / total_video_frames
        ...
        output_png_filepath = os.path.join(current_sequence_frames_dir, f"frame_{k:04d}.png")
        
        task_arg_tuple = (
            k, total_video_frames,
            iso4_path, iso6_path,
            ...
            particle_file, PARTICLE_SCALE_FACTOR, PARTICLE_CMAP,
            global_particle_diameter_range, SHOW_COLORBAR,
            args.ipd # Assicurati che args.ipd sia disponibile qui
        )
        worker_tasks_list.append(task_arg_tuple)

    print(f"Rendering {total_video_frames} frames using {NUM_PROCESSES} processes...")
    ...
    end_time = time.time()
    print(f"Sequence rendered in {end_time - start_time:.2f}s.")
    
    # Calcola il framerate corretto per FFMPEG
    # Se factor=1 -> 10 FPS. Se factor=3 -> 30 FPS.
    target_video_framerate = FRAMERATE * interpolation_factor
    
    final_video_path = os.path.join(OUTPUT_VIDEOS_ROOT_DIR, f"{sequence_base_name}.mp4")
    assemble_video_from_frames(current_sequence_frames_dir, final_video_path, target_video_framerate)
...
if __name__ == "__main__":
    ...
    ZOOM_FACTOR = 2.0 
    print(f"Applying Auto-Zoom x{ZOOM_FACTOR}...")
    calculated_top_down_view = zoom_camera(calculated_top_down_view, ZOOM_FACTOR)

    # Applichiamo lo zoom alla vista isometrica SOLO se non l'hai impostata a mano in interactive mode
    if not args.interactive:
        user_defined_isometric_base_view = zoom_camera(user_defined_isometric_base_view, ZOOM_FACTOR)    
    ...
    for angle in static_isometric_angles:
        create_animation_sequence(base_camera_view=user_defined_isometric_base_view,
                                  ...
                                  sequence_base_name=f"video_view_iso_{angle}deg",
                                  interpolation_factor=1)
    """
