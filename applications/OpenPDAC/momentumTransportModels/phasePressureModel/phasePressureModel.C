...
Foam::RASModels::phasePressureModel::continuousPhase() const
{
...
Foam::RASModels::phasePressureModel::pPrime() const
{
    ...
    const volScalarField alphaSolids
    (
        IOobject
        (
            IOobject::groupName("alphaSolids", U_.group()),
            mesh_.time().name(),
            mesh_,
            IOobject::NO_READ,
            IOobject::NO_WRITE
        ),
        scalar(1) - continuousPhase
    );

    const volScalarField alphaSolidsMax(phase_.fluid().alfasMax());
    ...
    (
        ...
        (
           ...
           *min
            (
                exp(preAlphaExp_*(alphaSolids - alphaSolidsMax)),
                expMax_
            )
        )
    );
