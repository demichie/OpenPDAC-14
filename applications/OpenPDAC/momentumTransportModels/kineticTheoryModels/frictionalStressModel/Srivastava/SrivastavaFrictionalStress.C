    ...
    const volSymmTensorField& D) const
{
    ...
    /*
        Srivastava frictional viscosity.
        ...
        In MFIX-style formulations, the frictional viscosity is commonly
        written directly in terms of pf_s/rho_s, because the solid stress tensor
        is not multiplied by an additional phase-volume-fraction factor.

        OpenPDAC instead keeps the phase volume fraction explicitly in the
